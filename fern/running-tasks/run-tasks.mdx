...
- Start with `skyvern-2.0` for most tasks — it handles complex flows and multi-step navigation.
- Use `skyvern-1.0` for straightforward tasks like single form fills or searches where speed matters more than capability.
- Use `openai-cua` or `anthropic-cua` if you have an existing provider contract and want that provider's native computer use model.
- Use `ui-tars` for visually rich desktop-style web UIs where screenshot-based navigation outperforms DOM parsing.

When using the Python SDK, pass `engine` directly to `run_task()` — see the [Python SDK Reference](/sdk-reference/python-sdk#engine-selection-guide).
