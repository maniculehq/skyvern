...
async def validate_skyvern_request_headers(request: Request) -> bool:
    header_skyvern_signature = request.headers["x-skyvern-signature"]
    payload = await request.body()  # bytes — must await in FastAPI
    hash_obj = hmac.new(
...
<Note>
**Replay protection:** Skyvern also sends an `x-skyvern-timestamp` header (Unix timestamp string). For production deployments, validate that the timestamp is within an acceptable window (e.g., ±5 minutes) before processing the payload:
...
def is_timestamp_fresh(request: Request) -> bool:
    ts = request.headers.get("x-skyvern-timestamp", "")
    try:
        return abs(time.time() - float(ts)) < TOLERANCE_SECONDS
    except ValueError:
        return False
```

Check the timestamp **before** computing the HMAC to avoid doing expensive work on obviously stale requests.
</Note>
