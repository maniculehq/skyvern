...
If you prefer to write the Python yourself — or want to run Skyvern entirely on your own machine — you can use the SDK without going through an AI coding agent.

<Tabs>
<Tab title="Local (no cloud required)">

Run Skyvern on your own machine with no API key. The `Skyvern.local()` constructor boots the engine in-process and auto-detects your LLM configuration from environment variables.

**Prerequisites:** an LLM API key (e.g. `OPENAI_API_KEY`) and Playwright (`pip install playwright && playwright install chromium`).
...
async def main():
    # Zero-config local mode: uses in-memory SQLite, detects LLM from env
    client = Skyvern.local()

    # Launch a visible Chromium window on your machine
    browser = await client.launch_local_browser(headless=False)
    page = await browser.get_working_page()

    # Mix standard Playwright steps with AI
    await page.goto("https://news.ycombinator.com")
    result = await page.agent.run_task(
        "Get the top comment of the #1 post",
        max_steps=10,
    )
    print(result.output)
...
<Tab title="Cloud (API key)">

Connect to Skyvern Cloud and run tasks in a managed browser session.
...
async def main():
    ...
    # Cloud-hosted browser — reuses an idle session if one exists
    browser = await client.use_cloud_browser()
    page = await browser.get_working_page()

    # Combine manual navigation with AI actions
    await page.goto("https://news.ycombinator.com")
    result = await page.agent.run_task(
        "Get the top comment of the #1 post",
        max_steps=10,
...
See the [Python SDK Reference](/sdk-reference/overview) for the full API.
