...
### Adaptive Caching eligibility

[Adaptive Caching](/sdk-reference/caching) automatically generates cached Python scripts for browser-interaction blocks after successful runs. Not all block types are cacheable:

**Cached automatically:** `navigation`, `action`, `extraction`, `login`, `file_download`, `for_loop` (and its child blocks), `task`, `task_v2`

**Never cached (always use AI at runtime):** `conditional`, `code`, `wait`, `text_prompt`, `http_request`, `send_email`, `validation`, `human_interaction`, `workflow_trigger`, `goto_url`, `file_url_parser`, `pdf_parser`, `print_page`, `upload_to_s3`, `download_to_s3`, `file_upload`

Set `disable_cache: true` on any cacheable block to opt it out of caching and always use the AI agent for that block.
...
<Tip>
**Writing effective navigation goals**

- Always include explicit COMPLETE and TERMINATE conditions. Without them, the agent may run too many steps or get stuck in a loop. Example: *"COMPLETE when the dashboard is visible. TERMINATE if a login page appears."*
- Reference parameters with `{{parameter_name}}` directly inside `navigation_goal`. Make sure `parameter_keys` lists the same names.
- Set `max_steps_per_run` to a reasonable upper bound (e.g., 20–30 for typical form flows). Lower values reduce cost; too low causes premature failure.
- Use `max_retries` (default 0) on flaky pages like those with rate-limiting or intermittent CAPTCHAs. Each retry restarts the block from `url`.
- `complete_on_download: true` is a shorthand for "COMPLETE when the download dialog appears." Use it with `download_suffix` to name the file.
</Tip>
...
<Note>
**Jinja2 vs. LLM evaluation**

Use `criteria_type: jinja2_template` when the condition depends on structured data (upstream block outputs, workflow parameters). Jinja2 expressions are evaluated instantly with no AI call. Use `criteria_type: prompt` when the condition requires understanding unstructured text or page state — this calls the LLM, adding latency and cost.

**Adaptive Caching and conditional blocks**: Conditional blocks themselves are _never_ cached — they always run via the AI agent so conditions are evaluated fresh. The action blocks _inside_ each branch are cached as normal and benefit from Adaptive Caching once that branch has been executed at least once.
</Note>
...
| `max_concurrency` | integer | `null` | Maximum number of iterations to run in parallel. `null` or `1` = sequential (default). Accepted range: 2–20. Added in v1.0.29. |
...
<Warning>
**For-loop limits and gotchas**

- **100-iteration hard cap**: For-loops stop after 100 iterations regardless of how many items are in the list. If your list is longer, split it into batches and chain multiple loops.
- **Empty list default**: If the list is empty and `complete_if_empty` is `false` (the default), the block fails. Set `complete_if_empty: true` to treat an empty list as a successful no-op.
- **Parallel execution requires fresh browser sessions**: When `max_concurrency > 1`, each iteration gets its own isolated browser. Persistent browser sessions cannot be shared across parallel iterations — if a `browser_session_id` is set on the workflow run, Skyvern automatically falls back to sequential execution.
- **Parallel execution is subject to org quota**: Your organization's parallel-session limit may cap `max_concurrency` below the value you set. If the quota is exhausted, Skyvern falls back to sequential execution rather than failing.
</Warning>
...
Trigger another workflow from within the current workflow. Use this to chain workflows together as modular building blocks — for example, a "master" workflow that fans out to several specialised sub-workflows. Added in v1.0.22.
...
```json JSON
{
  ...
  "workflow_permanent_id": "wf_abc123",
  "payload": {
...
| Parameter | Type | Default | Description |
|-----------|------|---------|-------------|
| `workflow_permanent_id` | string | — | **Required.** The permanent ID of the workflow to trigger. Found in the workflow URL or settings. Supports Jinja2 templates. |
| `payload` | object | — | Input parameters to pass to the triggered workflow. Values support Jinja2 templates. |
| `wait_for_completion` | boolean | `true` | If `true`, the current workflow pauses until the triggered workflow finishes. If `false`, the trigger fires and execution continues immediately. |
| `use_parent_browser_session` | boolean | `false` | If `true`, the triggered workflow shares the same live browser session as the parent. |
| `browser_session_id` | string | — | Explicit browser session ID to pass to the triggered workflow. |

**Output:** When `wait_for_completion` is `true`, the block output contains the triggered workflow's final output. Access it as `{{label_output}}` in downstream blocks.

<Info>
To find a workflow's `workflow_permanent_id`, open the workflow in the Skyvern UI. The ID appears in the URL: `https://app.skyvern.com/workflows/wf_abc123/...`
</Info>

<Note>
Using `use_parent_browser_session: true` allows sub-workflows to continue where the parent left off in the same browser tab. Use this for sequential handoff patterns. For parallel execution or isolated runs, leave it `false`.
</Note>
...
<CardGroup cols={2}>
  ...
  >
    Write workflows as Python scripts using the same block primitives
  </Card>
  <Card
  ...
  >
    Speed up repeated runs with @cached scripts
  </Card>
  <Card
