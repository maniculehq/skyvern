...
This page covers all 24 block types available for building workflows.
...
| [`workflow_trigger`](#workflow-trigger) | Trigger another workflow from within this workflow |
...
You can designate any block as a "finally" block that always executes when the workflow ends — whether it completed, failed, terminated, or timed out. The only exception is explicit cancellation via the API (`POST /v1/runs/{run_id}/cancel`).

**Use cases for finally blocks:**
- Send a notification email regardless of success or failure
- Close browser sessions or clean up resources
- Log workflow outcomes to an external system
...
| `max_concurrency` | integer | `null` | Run iterations in parallel. `null` or `1` = sequential. `2`–`20` = parallel batch size. |

<Info>
**Running loop iterations in parallel**

Set `max_concurrency` to a value between 2 and 20 to process multiple items simultaneously. Each value in the array runs in its own browser context. This is useful for bulk operations like downloading invoices for 50 vendors — instead of running one at a time, you can run 5 concurrently.

```yaml
- block_type: for_loop
  label: download_reports
  loop_over_parameter_key: vendor_list
  max_concurrency: 5  # Process 5 vendors at a time
  continue_on_failure: true
  loop_blocks:
    - block_type: navigation
      label: download_report
      url: "{{download_report.current_value.portal_url}}"
      navigation_goal: "Download the latest invoice"
```

**When NOT to use `max_concurrency`:** Avoid it when iterations share state (same browser session, same login) or when the target site rate-limits by IP. Sequential runs are safer for human-in-the-loop workflows.
</Info>
...
**When the loop item is an object (dict):** Each key from the object is accessible as a field on `current_value`:

```yaml
- block_type: for_loop
  label: process_vendors
  loop_over_parameter_key: vendors   # e.g. [{"name": "Acme", "portal_url": "https://..."}]
  loop_blocks:
    - block_type: navigation
      label: vendor_step
      url: "{{vendor_step.current_value.portal_url}}"
      navigation_goal: "Download the invoice for {{vendor_step.current_value.name}}"
```

**Renaming the loop variable:** Use `loop_variable_reference` to give the current item a more readable name instead of `{label}.current_value`. Set it in the for_loop block, then reference it by that name in nested blocks:

```yaml
- block_type: for_loop
  label: process_vendors
  loop_over_parameter_key: vendors
  loop_variable_reference: current_vendor   # accessible as {{current_vendor}}
  loop_blocks:
    - block_type: navigation
      label: vendor_step
      url: "{{current_vendor.portal_url}}"
      navigation_goal: "Download the invoice for {{current_vendor.name}}"
```

**Handling empty arrays:** By default, a `for_loop` over an empty array fails. Set `complete_if_empty: true` to treat an empty array as a successful no-op instead of a failure — useful when the array is fetched dynamically and may legitimately be empty.
</Info>
...
Use `http_request` to fetch data from an API and pass it to a `for_loop`:
...
- block_type: for_loop
  ...
  loop_over_parameter_key: fetch_orders_output   # array returned by the HTTP block
  loop_blocks:
    - block_type: navigation
      label: open_order
      url: "{{open_order.current_value.portal_url}}"
      navigation_goal: "Download the order PDF"
```

**Limitations:**
- Streaming responses are not supported — the block waits for the full response body.
- `files` (multipart upload) can reference `SKYVERN_DOWNLOAD_DIRECTORY` for files downloaded during the workflow, but cannot read arbitrary files from disk.
- The 30-second default timeout is suitable for most REST APIs. For slow endpoints, increase `timeout` up to a reasonable limit — very long timeouts block the workflow step.
...
The Code block runs in a restricted Python sandbox. Only the following names are available — you cannot use `import` statements.

**Available built-ins:**

| Name | Type | Notes |
|------|------|-------|
| `page` | Playwright `Page` | Live browser page. Requires browser-control access (see note below). |
| `len`, `range`, `str`, `int`, `float`, `dict`, `list`, `tuple`, `set`, `bool` | Python built-ins | Standard Python types. |
| `print` | function | Writes to the Skyvern log. Visible in `skyvern_log` artifact. |
| `sleep` | `async` function | `await sleep(seconds)` — pauses execution. |
| `asyncio.sleep` | `async` function | Alias for `sleep`. |
| `re` | module-like | `re.match`, `re.search`, `re.findall`, `re.sub`, `re.compile`, `re.split`, flags. |
| `json` | module-like | `json.dumps`, `json.loads`. |
| `Exception` | class | For raising or catching exceptions. |
| Each `parameter_key` | any | Values of the parameters listed in `parameter_keys`. |

**Security restrictions:**
- `import` statements are blocked entirely.
- Dunder attributes (`__foo__`) are blocked.
- OS, subprocess, network, and filesystem operations are blocked (e.g. `system`, `popen`, `listdir`, `open_connection`).

**Using the browser page:**

The `page` variable is the live Playwright `Page` object. Use it to interact with the browser directly.
...
- block_type: code
  ...
  code: |
    # Wait up to 5 seconds for a specific element to appear
    await page.wait_for_selector(".confirmation-message", timeout=5000)
    text = await page.inner_text(".confirmation-message")
    result = {"confirmation": text}
```

**Tip:** The `result` variable is what the block outputs. It can be any JSON-serializable value (`dict`, `list`, `str`, `int`, `bool`, `None`). If you never assign `result`, the block output is `None`.

<Note>
`page` access (browser control) is invite-only on Skyvern Cloud. Code blocks that don't use `page` (data transformation, string processing, arithmetic) are available to all plans. Contact support@skyvern.com for browser-control access.
</Note>
...
Trigger another workflow from within the current workflow. Use this to compose complex automations from smaller, reusable workflows.

When `wait_for_completion: true` (the default), the parent workflow pauses until the triggered workflow finishes, then continues with the child's output available to downstream blocks. When `wait_for_completion: false`, the child workflow is dispatched in the background and the parent continues immediately.

<CodeGroup>
```json JSON
{
  "block_type": "workflow_trigger",
  "label": "run_invoice_downloader",
  "workflow_permanent_id": "wpid_your_workflow_id",
  "payload": {
    "vendor_name": "{{ extract_vendor_output.vendor_name }}",
...
| Parameter | Type | Default | Description |
|-----------|------|---------|-------------|
| `workflow_permanent_id` | string | — | **Required.** The `wpid_*` ID of the workflow to trigger. |
| `payload` | object | `null` | Parameters to pass to the triggered workflow. Jinja2 templates in values are resolved against the parent's context before the call. |
| `wait_for_completion` | boolean | `true` | If `true`, the parent workflow waits for the child to finish before continuing. If `false`, the child is dispatched in the background and the parent moves on immediately. |
| `browser_session_id` | string | `null` | A specific browser session ID to pass to the triggered workflow. |
| `use_parent_browser_session` | boolean | `false` | If `true`, the triggered workflow shares the parent's active browser session, which is useful when the child needs to continue from the same authenticated browser state. |

**Block output (`{{ run_invoice_downloader_output }}`):**

| Field | Type | Description |
|-------|------|-------------|
| `workflow_run_id` | string | The run ID of the triggered workflow (`wr_*`) |
| `workflow_permanent_id` | string | The workflow ID that was triggered |
| `status` | string | Status of the triggered workflow. With `wait_for_completion: false`, always `queued`. |
| `failure_reason` | string \| null | Error message if the triggered workflow failed |
| `outputs` | object | The child workflow's named output parameters. Only present when `wait_for_completion: true`. |

Reference child workflow outputs in downstream blocks using dot notation:

```
{{ run_invoice_downloader_output.outputs.extracted_data }}
```

<Warning>
Workflow trigger chains are limited to 10 levels of nesting. Circular chains (workflow A triggers B which triggers A) are detected while A is still running and cause the trigger to fail.
</Warning>

<Note>
Async mode (`wait_for_completion: false`) requires Temporal and is only available on Skyvern Cloud. On self-hosted deployments without Temporal, the child workflow run record is created but execution does not proceed.
</Note>
