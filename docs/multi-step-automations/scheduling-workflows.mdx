...
<Note>
Schedule management (create, list, update, delete) is only available via the REST API or the Skyvern Cloud UI. The Python and TypeScript SDKs do not include schedule methods — use cURL, httpx, or any HTTP client directly.
</Note>
...
Send a `POST` request to `/v1/workflows/{workflow_permanent_id}/schedules` with your cron expression and timezone.
...
```bash cURL
curl -X GET "https://api.skyvern.com/v1/workflows/wpid_123456789/schedules/wfs_abc123" \
  -H "x-api-key: $SKYVERN_API_KEY"
```
...
```bash cURL
curl -X PUT "https://api.skyvern.com/v1/workflows/wpid_123456789/schedules/wfs_abc123" \
  -H "x-api-key: $SKYVERN_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
...
```bash cURL
# Disable
curl -X POST "https://api.skyvern.com/v1/workflows/wpid_123456789/schedules/wfs_abc123/disable" \
  -H "x-api-key: $SKYVERN_API_KEY"

# Re-enable
curl -X POST "https://api.skyvern.com/v1/workflows/wpid_123456789/schedules/wfs_abc123/enable" \
  -H "x-api-key: $SKYVERN_API_KEY"
```
...
```bash cURL
curl -X DELETE "https://api.skyvern.com/v1/workflows/wpid_123456789/schedules/wfs_abc123" \
  -H "x-api-key: $SKYVERN_API_KEY"
```
