...
| `failed` | System error or `max_steps` limit hit | Browser crash, network failure, or too many steps |
| `terminated` | AI gave up | Login blocked, CAPTCHA, page unavailable |
| `timed_out` | Exceeded wall-clock time limit | Task ran too long; check for infinite loops |
...
This indicates the task exceeded the wall-clock time limit (not the `max_steps` limit — that results in `failed`).

**Check:** `recording` (stuck in a loop?) and `step_count` (how many steps taken?)

**Fix:**
- Add explicit completion criteria: "COMPLETE when you see 'Order confirmed'"
- Break complex tasks into multiple workflow blocks so each completes sooner
- If the task genuinely needs more steps before the clock runs out, set `max_steps` to a lower value so you get a `failed` result earlier with diagnostic information
</Accordion>
