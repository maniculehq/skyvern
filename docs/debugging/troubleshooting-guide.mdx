...
<Note>
**Check `failure_category` first.** On task_v2 runs, Skyvern automatically classifies the failure into categories like `AUTH_FAILURE`, `ANTI_BOT_DETECTION`, or `ELEMENT_NOT_FOUND`. These appear in `run.failure_category` before you dig into artifacts. See [Failure categories](/going-to-production/error-handling#failure-categories) for the full reference and how to act on each category.
</Note>
...
# For task_v2 runs: machine-readable failure categories
if run.failure_category:
    top = run.failure_category[0]
    print(f"Failure category: {top['category']} (confidence: {top['confidence_float']})")
```
...
// For task_v2 runs: machine-readable failure categories
if (run.failure_category?.length) {
  const top = run.failure_category[0];
  console.log(`Failure category: ${top.category} (confidence: ${top.confidence_float})`);
}
```
...
curl -X GET "https://api.skyvern.com/v1/runs/$RUN_ID" \
  ...
  | jq '{status, failure_reason, step_count, output, failure_category}'
```
</CodeGroup>
