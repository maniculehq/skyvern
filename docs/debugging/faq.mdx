...
2. **`failure_category`** — On task_v2 runs, check `run.failure_category` for machine-readable classification (e.g., `AUTH_FAILURE`, `ANTI_BOT_DETECTION`, `ELEMENT_NOT_FOUND`). This is faster than parsing `failure_reason` text
3. **Timeline** — Use `get_run_timeline(run_id)` to find which block failed
4. **Artifacts** — Check `screenshot_final` to see where it stopped, and `recording` to watch what happened
...
See the [Troubleshooting Guide](/debugging/troubleshooting-guide) for detailed steps and [Failure categories](/going-to-production/error-handling#failure-categories) for how to act on each category.
</Accordion>
