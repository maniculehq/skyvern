...
- **`failed`** — System error (browser crash, network failure) or hit the `max_steps` limit
...
<Accordion title="Why did my task fail with status: failed?">
Tasks fail when they hit an error or exceed `max_steps`. Hitting `max_steps` sets status to `failed` (not `timed_out`). This happens when:
