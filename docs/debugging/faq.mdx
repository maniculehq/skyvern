...
result = await client.run_task(
    prompt="...",
    url="https://example.com",
    proxy_location="RESIDENTIAL"  # or RESIDENTIAL_ISP for static IPs
)
```

Available locations include `RESIDENTIAL` (United States), `RESIDENTIAL_GB` (United Kingdom), `RESIDENTIAL_ISP`, and more. Use `RESIDENTIAL_ISP` for sites that require consistent IP addresses.
</Accordion>
