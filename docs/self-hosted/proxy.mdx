...
<Note>
Skyvern Cloud supports a `proxy_location` parameter on task requests for geographic targeting (e.g., `RESIDENTIAL` for the United States). This feature is not available in self-hosted deployments. All tasks use the proxy configured in `HOSTED_PROXY_POOL`.
</Note>
...
```bash
curl -s -X POST http://localhost:8000/v1/run/tasks \
