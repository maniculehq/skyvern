...
Use `enum` to constrain values to a fixed set — this reduces hallucination on categorical fields:
...
Use `description` on the schema root object (not just properties) to give the AI extraction context:

```python
data_extraction_schema={
    "type": "object",
    "description": "Extract from the invoice detail page. All amounts are in USD. Dates use ISO 8601 format (YYYY-MM-DD).",
    "properties": {
        "invoice_number": {
            "type": "string",
            "description": "Invoice number shown at top of document, typically starts with 'INV-'"
        },
        "amount_due": {
            "type": "number",
            "description": "Total amount due in USD, without currency symbol or commas"
        }
    }
}
```

To extract a flat list (without a wrapper object), pass an array schema directly:
