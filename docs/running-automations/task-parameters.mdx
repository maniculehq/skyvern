...
A label that identifies which TOTP code to use when Skyvern encounters a 2FA prompt. You push the code to Skyvern's API yourself using this identifier — Skyvern does not fetch it. Use `totp_identifier` when your application generates the OTP and sends it to Skyvern on demand.

See [Handle 2FA](/credentials/handle-2fa) for the full push-code flow.
...
A URL on your server that Skyvern polls to fetch a fresh TOTP code whenever a 2FA prompt appears. Use `totp_url` when your server generates the OTP — Skyvern calls this endpoint automatically during execution and you don't need to push codes manually.

See [Handle 2FA](/credentials/handle-2fa) for the expected response format from your endpoint.
...
<CardGroup cols={2}>
  ...
  >
    Configure TOTP and two-factor authentication for automated logins
  </Card>
  <Card
  ...
  >
    Map failures to custom error codes for programmatic handling
  </Card>
</CardGroup>
