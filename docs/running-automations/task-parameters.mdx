...
A label that links this task to 2FA codes you push to Skyvern via API. When Skyvern encounters a 2FA prompt during login, it looks up the most recent code pushed under this identifier.

Use this when you receive codes via email or SMS and forward them to Skyvern. See [Handle 2FA](/credentials/handle-2fa#method-2-push-codes-to-skyvern) for setup instructions.
...
An endpoint that Skyvern calls to retrieve 2FA codes on demand. When Skyvern encounters a 2FA prompt during login, it sends a POST request to this URL and expects the verification code in the response.

Use this when your system receives codes and can expose them via HTTP. Skyvern polls every 10 seconds until a code is returned. See [Handle 2FA](/credentials/handle-2fa#method-3-skyvern-pulls-from-your-endpoint) for the expected request/response format and signature verification.
...
Pass the full action history to the Validator when it checks whether the task goal was achieved. Default: `false`.

By default, the Validator only sees the current page screenshot. Setting this to `true` also passes the sequence of actions taken, which helps the Validator confirm that multi-step goals were actually completed — for example, that a form was filled and submitted rather than just visited. Useful when the final page state alone is ambiguous (a confirmation page that looks the same whether you filled the form or navigated there directly).
