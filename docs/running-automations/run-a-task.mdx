...
Skyvern sends a POST request with the full run data when the task completes or fails. See [Webhooks](/going-to-production/webhooks) for the payload format and signature verification.
...
The run response contains high-level data like `output`, `recording_url`, and `screenshot_urls`. Every run also generates detailed artifacts — screenshots, reasoning logs, downloaded files — for observability. To retrieve them, use `get_run_artifacts`.
