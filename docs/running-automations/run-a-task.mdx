...
| `failed` | Task encountered an error (including hitting `max_steps` limit)—check `failure_reason` and retry or adjust your prompt |
| `terminated` | AI determined the goal is unachievable—check `failure_reason` for details |
| `timed_out` | Task exceeded the wall-clock time limit |
...
| `failure_reason` | string \| null | Error description if the run failed or terminated |
...
The run response includes the key results: `output`, `recording_url`, and `screenshot_urls`. Every run also captures detailed artifacts — screenshots at each step, AI reasoning logs, downloaded files — useful for debugging. To fetch them, call `get_run_artifacts`.
