...
| `run_type` | string | Type of run. See table below. |
...
| `script_run` | object \| null | Present when a generated script ran (code-generation mode). Contains `script_run_id` and `ai_fallback` (whether the AI agent took over when script failed). |

**The `errors` array:**

The `errors` array contains per-step errors that occurred during execution. Even a `completed` run can have non-empty `errors` if the agent recovered from an error mid-task. Each entry has this shape:
  ...
  "errors": [
    {
      ...
      "error_message": "Could not locate the Submit button on the page",
...
If `retried: true`, the agent encountered the error but recovered and continued. If `retried: false`, the error contributed to the final failure state. Use `errors` for debugging step-level issues — the `failure_reason` on the run itself describes the terminal outcome, while `errors` shows what went wrong along the way.

**`run_type` values:**

| Value | When you see it |
|-------|----------------|
| `task_v2` | Default for tasks run via `POST /v1/run/tasks` |
| `task_v1` | Legacy tasks (uncommon; created via the older v1 API) |
| `openai_cua` | Task run with the `openai-cua` engine (OpenAI Computer Use Agent) |
| `anthropic_cua` | Task run with the `anthropic-cua` engine (Anthropic Computer Use Agent) |
| `ui_tars` | Task run with the `ui-tars` engine (ByteDance UI-TARS model) |
| `workflow_run` | Workflow runs returned from `GET /v1/runs` alongside task runs |

To use a CUA engine, pass `engine` when creating the task:
...
curl -X POST "https://api.skyvern.com/v1/run/tasks" \
  ...
  -H "Content-Type: application/json" \
  -d '{
    "prompt": "Book the cheapest flight from NYC to LAX next Tuesday",
    "url": "https://google.com/flights",
    "engine": "anthropic-cua"
  }'
```

CUA engines (`openai-cua`, `anthropic-cua`, `ui-tars`) delegate browser control directly to the respective model's native computer-use capability rather than Skyvern's own perception-action loop. This is useful when a specific model's built-in web navigation performs better for your task. CUA runs have the same response shape and status lifecycle as standard tasks.
...
Every run also generates artifacts — screenshots, reasoning logs, downloaded files — for observability and debugging. To retrieve them, use `get_run_artifacts`. For guidance on using artifacts to diagnose failures, see [Using Artifacts](/debugging/using-artifacts).
