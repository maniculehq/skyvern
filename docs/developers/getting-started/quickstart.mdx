...
<Tip>
**Skip the server with `Skyvern.local()`.** For Python scripts and notebooks, the SDK can boot Skyvern in-process. No `skyvern run server`, no Postgres required, with auto-fallback to in-memory SQLite when no database is configured.
...
See [SDK Reference → Local mode](/sdk-reference/overview#local-mode) for the supported / unsupported feature list (scheduling, cloud sessions, rate limiting, and multi-worker execution require Postgres).
</Tip>
