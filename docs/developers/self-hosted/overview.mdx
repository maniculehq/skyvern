...
- **Three settings cannot be overridden in embedded mode.** `OTEL_ENABLED` and `ENABLE_CLEANUP_CRON` are forced to `False`, and `DATABASE_STRING` is rewritten to a SQLite connection (file or in-memory). Pass `use_in_memory_db=True` to keep nothing on disk; otherwise the SQLite file at `~/.skyvern/data.db` is used. Settings are snapshotted before bootstrap and restored on `aclose()`, so a mid-test failure will not leave global config in an inconsistent state.
