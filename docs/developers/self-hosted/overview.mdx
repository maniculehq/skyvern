...
Most teams start with Docker Compose. It has the smallest setup surface for a working deployment. Move to Kubernetes when you need horizontal scaling or want to integrate with existing orchestration infrastructure.

## Next steps

Start with one of the deployment paths, then come back for the optional tuning pages once your stack is up.
...
<CardGroup cols={2}>
  ...
  <Card title="Credential Vaults" icon="vault" href="/developers/credentials/credential-vaults">
    Pick a backend for credential storage (Bitwarden, Azure Key Vault, 1Password, custom)
  </Card>
  <Card title="Cleanup Cron" icon="broom" href="/developers/self-hosted/cleanup-cron">
    Reclaim temp files and kill stale browser processes on long-lived hosts
  </Card>
</CardGroup>
