...
Script generation is *not* a per-call SDK parameter. There are two ways to turn it on, both at the workflow level:

- **Set `generate_script_on_terminal=True` on the workflow definition** (`skyvern/schemas/workflows.py:1178`). Whenever a run of that workflow finishes in `completed` or `terminated` state, Skyvern emits a script.
- **Run a Task v2 with `publish_workflow=True`** on the task request (`skyvern/library/skyvern.py:294`). The successful v2 run is materialized as a workflow *and* recorded as that workflow's first script:
