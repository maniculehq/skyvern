...
When a workflow runs successfully on a real site, Skyvern can capture what the agent did and emit it as a Playwright script you can replay deterministically. Subsequent code-mode runs that match the same shape skip the LLM and execute the script directly, reducing inference cost on repeated workflows. By the end of this page, you'll know how scripts are produced, how cache keys decide when to use one, and how to pin, edit, or invalidate them.

## Why generated scripts exist

The agent is useful when the page is unfamiliar and wasteful when it has to redo the same path a hundred times. A generated script is the bridge: the first eligible run pays the LLM cost to learn the path, later code-mode runs execute the recorded code. The LLM only re-enters the loop when the page actually changes and fallback is enabled.
...
Script generation is *not* controlled by a `generate_script` parameter on each SDK run call. There are two public ways to turn it on:

- **Set `generate_script_on_terminal=True` on the workflow definition** (`skyvern/schemas/workflows.py:1178`). Completed workflow runs can generate scripts in the normal post-run path; this flag also makes `terminated` runs script-eligible. Failed, canceled, and timed-out runs skip generation.
- **Run a Task v2 with `publish_workflow=True` and `run_with="code"`** on the task request (`skyvern/library/skyvern.py:294`). The successful v2 run is materialized as a workflow and becomes that workflow's first script:

```python
result = await client.run_task(
    prompt="Find the report for last quarter and email it to me.",
    engine="skyvern-2.0",
...
Internally the workflow service flips a request-scoped `generate_script` flag on the `SkyvernContext` (`skyvern/forge/sdk/workflow/service.py:1340-1350`) so the agent loop knows to record what it does. You don't set that flag yourself; the two switches above are the only public surface.
...
When a workflow run starts, Skyvern looks up `(workflow_permanent_id, cache_key_value)`. If a script is published for that pair, it replays. If not, the agent runs and (if the workflow has `generate_script_on_terminal=True`, or the run came from a v2 task with `publish_workflow=True` and `run_with="code"`) the run becomes the script for next time.
...
By default the latest published version is replayed. When you want to lock to one, say you've audited v3 and don't trust auto-promotion, pin it via the HTTP API. The pin and unpin endpoints are not in the public OpenAPI schema (`include_in_schema=False`) and are not exposed on the Python SDK; call them directly with the `x-api-key` header:

```bash
curl -X POST \
  -H "x-api-key: $SKYVERN_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"cache_key_value": "https://example.com"}' \
  https://api.skyvern.com/v1/scripts/wpid_abc123/pin
```

Endpoint: `POST /v1/scripts/{workflow_permanent_id}/pin` (`skyvern/forge/sdk/routes/scripts.py:1186-1221`). Unpin with the matching `/unpin` endpoint and the same `cache_key_value` body.
...
When a replay can't proceed because the page drifted, Skyvern records a *fallback episode* (`ScriptFallbackEpisodeModel`, `db/models.py:1298-1349`). When you've accumulated enough of those to be sure the script needs help, ask the reviewer to fix it. Like the pin endpoint, the review endpoint is internal-only (`include_in_schema=False`) and not exposed on the Python SDK; call it over HTTP:
...
curl -X POST \
  ...
  https://api.skyvern.com/v1/scripts/wpid_abc123/review
```
...
- `DELETE /v1/scripts/{workflow_permanent_id}/value?cache-key-value=<value>` removes a single cache-key value's script binding.
- `DELETE /v1/scripts/{workflow_permanent_id}/cache` clears all cached scripts for the workflow at once (`scripts.py:1126-1173`).

Both are soft deletes against `WorkflowScriptModel` plus an in-memory cache flush. The next run that satisfies the generation rules above will produce a new version from scratch.
...
<CardGroup cols={2}>
  ...
  >
    Trigger workflow runs and pass `run_with="code"`.
  </Card>
</CardGroup>
