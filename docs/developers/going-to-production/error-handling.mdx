...
| `failed` | System error (browser crash, network failure, exception) or a budget cap fired (`MAX_STEPS_PER_RUN`, `MAX_RETRIES_PER_STEP`). Read `failure_reason` to tell them apart. |
| `terminated` | AI determined the goal is unachievable (login blocked, page unavailable) |
| `timed_out` | The workflow run was timed out (e.g. by an external scheduler), which cascaded to the task |
...
### Two ways a run ends up `failed`: budget vs retry

Two server-config knobs both bound a run, and both surface as `status=failed` with different `failure_reason` text. Reading the reason is how you tell which knob tripped:

| Setting | Default | Trigger condition | `failure_reason` starts with |
|---------|---------|-------------------|------------------------------|
| `MAX_STEPS_PER_RUN` (`max_steps` on the call) | 10 | Successful step count reached the cap. | `"Reached the maximum steps (...)"` (task v1) or `"Reached the max number of N steps."` (task v2) |
| `MAX_RETRIES_PER_STEP` | 5 | A single step has failed its retry budget. | `"Max retries per step (...) exceeded"` |

Retries do not consume your step budget — Skyvern creates a new `Step` row with the same `order` and `retry_index + 1`, and only advances `order` on success. So a 10-step run can in principle make `10 × (1 + 5) = 60` agent cycles before exiting, but only the 10 successful ones count against `max_steps`. Two things follow:

- A run with `Max retries per step` in the reason is stuck on a *single* step. Look at the screenshots near the run's tail to see which one. Raising `max_steps` will not help — fix the prompt, the credentials, or the page selector.
- A run with `Reached the maximum steps` in the reason got through real steps but ran out of room. Either raise `max_steps` or split the task into shorter `agent.run_task` calls so each gets its own budget.

`status=timed_out` is a separate signal: it is set at the workflow-run layer (for example by an external scheduler timeout) and propagates down to the task. Reaching `MAX_STEPS_PER_RUN` does not produce `timed_out`.
