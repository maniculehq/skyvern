...
| `run_type` | string | Type of run: `task_v1`, `task_v2`, `openai_cua`, `anthropic_cua`, `ui_tars`, or `workflow_run` |
...
- **Timeout**: Skyvern waits 30 seconds for a response. If your server takes longer, the delivery is marked as failed even if processing eventually succeeds. Return `200 OK` immediately and process the payload in a background job. The replay endpoint (`retry_run_webhook`) uses a separate 60-second timeout.
