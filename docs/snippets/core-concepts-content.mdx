...
For detailed block configuration, see [Workflow Blocks](/multi-step-automations/workflow-blocks-reference).
...
```bash cURL
curl -X POST "https://api.skyvern.com/v1/workflows/wpid_abc123/schedules" \
...
<Note>
Schedule management (create, list, update, delete) is only available via the REST API or the Skyvern Cloud UI. There is no Python or TypeScript SDK wrapper for schedule endpoints. See [Scheduling Workflows](/multi-step-automations/scheduling-workflows) for full details.
</Note>
