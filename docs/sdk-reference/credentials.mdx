...
Overwrite the stored credential data (e.g. username/password) while keeping the same `credential_id`.
...
| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `credential_id` | `str` | Yes | The ID of the credential to update. |
| `name` | `str` | Yes | Display name for the credential. |
| `credential_type` | `CredentialType` | Yes | Type of credential. |
| `credential` | `CreateCredentialRequestCredential` | Yes | The new credential data. Shape depends on `credential_type`. |
