...
| **Script functions** (`skyvern` top-level) | `from skyvern import run_task, extract, loop, ...` | Writing `@workflow`-decorated scripts. These async functions map directly to workflow blocks and are used inside the Adaptive Caching scripting layer. |
| **TypeScript SDK** (`@skyvern/client`) | `import { SkyvernClient } from "@skyvern/client"` | Node.js, Vercel, Cloudflare Workers, Deno, or Bun applications. Mirrors the Python REST client API surface. |

### Decision guide

```
Do you need browser automation (clicks, forms, downloads)?
├── Yes, in Python with Playwright-style page control → skyvern.library (Skyvern.local() or Skyvern(api_key=...))
├── Yes, inside a @workflow script using block functions → skyvern top-level (run_task, extract, loop, ...)
└── No, I just need to trigger runs and read results via REST:
    ├── Python → skyvern.client (Skyvern)
    └── TypeScript/JS → @skyvern/client (SkyvernClient)
```
