...
## Which SDK surface should I use?

Skyvern exposes three distinct Python SDK surfaces and one TypeScript SDK. Use this table to pick the right one for your use case:

| Surface | Import | Best for |
|---------|--------|----------|
| **REST client** (`skyvern.client`) | `from skyvern.client import Skyvern` | Integrating Skyvern Cloud into any Python application. Wraps all REST endpoints in typed async methods. Auto-generated from the OpenAPI spec. |
| **Library SDK** (`skyvern.library`) | `from skyvern import Skyvern` | Playwright-style embedded automation. Boots Skyvern locally (`Skyvern.local()`) or connects to Cloud. Supports `page.goto()`, `page.agent.run_task()`, and direct browser control. |
| **Script functions** (`skyvern` top-level) | `from skyvern import run_task, extract, loop, ...` | Writing [`@workflow`-decorated scripts](/sdk-reference/caching). These async functions map directly to [workflow blocks](/multi-step-automations/workflow-blocks-reference) and are used inside the Adaptive Caching scripting layer. See the [Script Functions reference](/sdk-reference/script-functions). |
| **TypeScript SDK** (`@skyvern/client`) | `import { SkyvernClient } from "@skyvern/client"` | Node.js, Vercel, Cloudflare Workers, Deno, or Bun applications. Mirrors the Python REST client API surface. See the [TypeScript SDK reference](/ts-sdk-reference/overview). |

### Decision guide

```
Do you need browser automation (clicks, forms, downloads)?
├── Yes, in Python with Playwright-style page control
│   → skyvern.library  →  Skyvern.local()  or  Skyvern(api_key=...)
├── Yes, inside a @workflow script using block functions
│   → skyvern top-level  →  run_task, extract, loop, ...  (see Script Functions)
└── No, I just need to trigger runs and read results via REST:
    ├── Python      → skyvern.client  →  Skyvern
    └── TypeScript  → @skyvern/client  →  SkyvernClient
```

<Tip>
Writing scripts with `@workflow` also unlocks [Adaptive Caching](/sdk-reference/caching) — Skyvern learns from successful runs and replaces AI calls with fast cached Python functions on future runs.
</Tip>
...
- `Skyvern.local()` requires a running PostgreSQL or SQLite database (configured in `.env`). If the database is not initialized, you'll get a connection error. Run `skyvern quickstart` or `alembic upgrade head` first.
- Starting with v1.0.29, `skyvern run server` uses SQLite by default, so local development no longer requires PostgreSQL. If you're on an older version, you need the Docker stack (`docker-compose up`) or a local PostgreSQL instance.
- `Skyvern.local()` boots the Skyvern engine in-process. Only call it once per Python process — multiple calls create duplicate browser managers and may conflict.
- If you're using local mode in tests, call `await client.close()` (or use the client as an async context manager: `async with Skyvern.local() as client: ...`) to properly clean up browser resources.
</Warning>
