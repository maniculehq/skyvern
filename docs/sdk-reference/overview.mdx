...
async def main():
    client = Skyvern.local()  # synchronous — no await needed
...
| `use_in_memory_db` | `bool \| None` | `None` | `None` = auto-detect (use PostgreSQL if `DATABASE_STRING` is set, otherwise SQLite). `True` = always use in-memory SQLite. `False` = always use PostgreSQL. |
...
| `timeout` | `float` | `1800` | Maximum wait time in seconds. Raises `asyncio.TimeoutError` if exceeded. |
