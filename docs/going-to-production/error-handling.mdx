...
| `failed` | System error (browser crash, network failure, exception) or `max_steps` limit hit |
| `terminated` | AI determined the goal is unachievable (login blocked, page unavailable) |
| `timed_out` | Exceeded the wall-clock time limit |
...
**`failed` vs `terminated`:** A `failed` run hit infrastructure problems or exceeded `max_steps`—retrying may work if the cause was transient, or increase `max_steps` if the task needs more steps. A `terminated` run means the AI recognized the goal is unachievable with current conditions. Retrying without changes (new credentials, different URL) will produce the same result.
