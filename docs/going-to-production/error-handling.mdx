...
The `status` field is at the top level of both responses. Poll `get_run` with the `run_id` until it reaches a terminal state. This works identically for task runs and workflow runs:
...
Use the `run_id` (for tasks) or `workflow_run_id` (for workflows) — both go through the same `get_run` endpoint:
