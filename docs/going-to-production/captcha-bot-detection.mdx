...
Skyvern detects CAPTCHAs using its vision model and solves them automatically. Solving is not guaranteed — solvers can fail on novel challenges or rate-limited IPs. This guide covers how to detect CAPTCHA failures, implement manual fallbacks, and reduce bot-detection triggers in the first place.
...
## Map CAPTCHA failures to error codes

When a CAPTCHA blocks your automation, use `error_code_mapping` to get a consistent error code instead of parsing free-text failure messages. See [Error Handling](/going-to-production/error-handling#step-3-use-error_code_mapping) for the full pattern.
