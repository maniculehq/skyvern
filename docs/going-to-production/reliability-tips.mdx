...
<Warning>
**Validation blocks read from the current page, not from prior block outputs.** If you need to validate extracted data from a previous block, use a [Code block](/multi-step-automations/workflow-blocks-reference#code) to compare values programmatically. Validation blocks evaluate their criteria against what's visible on screen—they don't have access to data extracted by earlier extraction blocks.
</Warning>
