...
Skyvern Cloud uses a **credit-based billing model**. Each step — one AI decision and action cycle — draws from your monthly credit allowance. The strategies on this page reduce consumption directly: capping steps with `max_steps` to prevent runaway costs, eliminating AI inference on repeated tasks through code generation, and choosing a lighter engine when a task doesn't need maximum capability.
