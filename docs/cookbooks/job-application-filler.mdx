...
description: Build a workflow that parses your resume, searches job portals, extracts listings, generates tailored answers with AI, and submits applications automatically.
...
<img src="/images/cookbooks/job-stash.png" alt="Job Stash, a demo job portal" />
---
  ...
  <Tab title="Cloud UI">
    <Steps>
      ...
      <Step title="Create a credential">
        ...
        <img src="/images/cookbooks/js-creds.png" alt="Fill username and password to add a credential to Skyvern" />
      </Step>
      <Step title="Copy the credential ID">
        Copy the credential ID (here: `cred_504284876895871734`). You'll use this when configuring the workflow.
  ...
  <Tab title="Cloud UI">
    <Steps>
      <Step title="Create the workflow">
        Go to **Workflows** → **Create Workflow**. Name it "Search and Apply for Jobs Workflow".
        <img src="/images/cookbooks/create-workflow.png" alt="Create a new workflow from Skyvern Dashboard" />
      </Step>
    </Steps>
  </Tab>
  <Tab title="API / SDK">
    A **workflow definition** is a JSON or YAML file that describes your entire workflow: its metadata, parameters, and blocks. Create a file called `job-application-workflow.yaml` (or `.json`) with the following template:
...
<Tabs>
  ...
  <Tab title="API / SDK">
    ...
    <CodeGroup>
    ```jsonc JSON
    // Replace the empty `parameters` array with these entries:
    "parameters": [
...
<Tabs>
  <Tab title="Cloud UI">
    ...
    <Steps>
      <Step title="Create the workflow">
        Go to **Workflows** → **Create Workflow**. Name it "Search and Apply for Jobs Workflow." On the **Start** node, set **Proxy Location** to **Residential** and add the parameters: `resume`, `credentials`, and `job_portal_url`.
      </Step>
