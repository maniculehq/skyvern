...
When a session reaches its timeout, streaming and renewal checks can treat it as invalid. Long workflow runs try to renew active sessions every 5 minutes for up to 2 hours, but you should still set timeouts with enough margin for your longest expected task.
...
If your steps run back-to-back with no pauses between them, a [workflow](/cloud/building-workflows/build-a-workflow) is usually the better fit. Workflow blocks share one browser instance with no inter-step startup cost.
...
A session is a live browser; a profile is a saved snapshot of one. Use a session when you need a browser right now (back-to-back tasks, human-in-the-loop). Use a [profile](/cloud/browser-management/browser-profiles) when you want to skip a login on later runs.

<Tip>
For the full comparison, lifecycle diagram, and a picking rule of thumb, see [Profiles vs Sessions](/cloud/browser-management/profiles-vs-sessions).
</Tip>
...
<CardGroup cols={2}>
  <Card
    ...
    icon="scale-balanced"
    href="/cloud/browser-management/profiles-vs-sessions"
  >
    Decide when to keep a browser open vs save its state for reuse
  </Card>
  <Card
