...
A profile is the snapshot a session leaves behind. Use a profile when the work happens later (scheduled workflows, repeated logins). Use a [session](/cloud/browser-management/browser-sessions) when the work happens now and benefits from a warm browser.
...
<CardGroup cols={2}>
  ...
  >
    Decide when to save state and when to keep a browser open
  </Card>
  <Card
