...
A session is the *thing the browser is doing right now*. A profile is the *paper trail* it leaves behind. You almost always create a profile from a session. You rarely create one from scratch.
...
| Column | Meaning |
|---|---|
| `status` | `created`, `running`, `completed`, etc. — a session has a lifecycle. |
...
A `BrowserProfileModel` (lines 982-998) is the snapshot. It has a `name` unique per organization and points back to the run or session it was captured from. There are no compute fields because a profile is just data sitting in storage.
...
Sessions auto-close when their timeout expires, even mid-run. Pad your timeouts.
...
Profiles expire only as fast as the underlying cookies and session tokens do. Some sites stay logged in for months; banks and healthcare portals may force re-auth in hours. If the agent suddenly hits a login page mid-run, refresh the profile.
